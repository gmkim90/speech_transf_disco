...
class Trainer(object):
    ...
    def build_model(self):
        ...
        #elif (self.dataset == 'chime3_50' or self.dataset == 'chime3_100'):
    ...
    def train(self):
        ...
        for step in trange(self.start_step, self.max_step):
            ...
            if x_A.size(0) != x_B.size(0):
                #pdb.set_trace()
                print("[!] Sampled dataset from A and B have different # of data." + str(x_A.size(0)) + ' VS ' + str(x_B.size(0)) + ". Try resampling...")
                continue
    ...
    def generate_wav(self):
        ...
        wav_dir = os.path.join(self.model_dir, 'wav')
        ...
        for i in range(3):  # repeat 3 times for example
            x_A, x_B = self._get_variable(next(A_loader)), self._get_variable(next(B_loader))
            ...
            # Generate wav
            print('Generate & Save wav ' + str(i+1) + '/3')
            savename = 'A' + str(i); self.genwav_save(x_A, savename)
            savename = 'AB' + str(i); self.genwav_save(x_AB, savename)
            savename = 'B' + str(i); self.genwav_save(x_B, savename)
            savename = 'BA' + str(i); self.genwav_save(x_BA, savename)
    ...
    def genwav_save(self, spec, savename):
        if self.config.use_log_magnitude:  # maybe false
            audio = spectrogram2wav(np.power(np.e, spec)**self.config.power)
        else:
            spec = np.where(spec < 0, 0, spec)
            audio = spectrogram2wav(spec**self.config.power)
        write(savename, self.config.fs, audio)
