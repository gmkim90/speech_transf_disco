...
def pix2pix_split_images(root):
    ...
    for path in tqdm(paths, desc="pix2pix processing"):
        ...
        #pdb.set_trace()
        a_image = Image.fromarray(data[:,:int(width/2)].astype(np.uint8))
        b_image = Image.fromarray(data[:,int(width/2):].astype(np.uint8))

        a_image.save(a_image_path)
        b_image.save(b_image_path)

class Dataset(torch.utils.data.Dataset):
    def __init__(self, root, scale_size, data_type, skip_pix2pix_processing=False, disallow_resize=False, grayImage=False):
        self.root = root
        ...
        if grayImage:
            #print('grayImage')
            self.shape = list(Image.open(self.paths[0]).size) + [1]
        else:
...
def get_loader(root, batch_size, scale_size, num_workers=2,
               skip_pix2pix_processing=False, disallow_resize=False, grayImage=False):
    #pdb.set_trace()
    a_data_set, b_data_set = \
        Dataset(root, scale_size, "A", skip_pix2pix_processing, disallow_resize, grayImage), \
        Dataset(root, scale_size, "B", skip_pix2pix_processing, disallow_resize, grayImage)
